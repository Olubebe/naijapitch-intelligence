...
const emailFrom =
  ...
  "NaijaPitch Intelligence <no-reply@naijapitch.local>";
...
const DEFAULT_LINK_EXPIRY_HOURS = Number(
  process.env.DEFAULT_LINK_EXPIRY_HOURS || 48,
);
...
function buildClubAliases(club: { id: string; name: string }) {
  ...
  const slugName = normalizeSearchText(
    String(club.id || "").replace(/-/g, " "),
  );
...
function detectClubFeedbackContext(
    ...
    .map((club) => {
      const score = club.aliases.reduce((sum, alias) => {
        const regex = new RegExp(
          `\\b${escapeRegex(alias).replace(/\s+/g, "\\s+")}\\b`,
          "g",
        );
  ...
  const isCrossClubFeedback = Boolean(topMatch && topMatch.id !== ownerClubId);
  ...
  return {
    ...
    primarySubjectClubName:
      clubs.find((club) => club.id === primarySubjectClub)?.name ||
      ownerClub?.name ||
      null,
    mentionedClubs,
...
function normalizeTopicType(value: unknown) {
  ...
  return ["match", "transfer", "players"].includes(sanitized)
    ? sanitized
    : "match";
}
...
function buildFeedbackHighlights(rows: any[]) {
  const concerns = [...rows]
    ...
    .sort(
      (a, b) => Number(a.sentiment_score || 0) - Number(b.sentiment_score || 0),
    )
    .slice(0, 2)
  ...
  const positives = [...rows]
    .filter((row) => Number(row.sentiment_score || 0) >= 0.2)
    .sort(
      (a, b) => Number(b.sentiment_score || 0) - Number(a.sentiment_score || 0),
    )
    .slice(0, 2)
    .map((row) => sanitizeText(row.translated_text || row.original_text, 220));
...
function buildLinkDigest(match: any, rows: any[]) {
  ...
  const avgSentiment = total
    ? rows.reduce((sum, row) => sum + Number(row.sentiment_score || 0), 0) /
      total
    : 0;
  const avgCredibility = total
    ? rows.reduce((sum, row) => sum + Number(row.credibility_score || 0), 0) /
      total
    : 0;
  const flaggedCount = rows.filter(
    (row) => row.validation_status === "FLAGGED",
  ).length;
  ...
  const sentimentMix = {
    positive: rows.filter(
      (row) =>
        getSentimentBand(Number(row.sentiment_score || 0)) === "positive",
    ).length,
    neutral: rows.filter(
      (row) => getSentimentBand(Number(row.sentiment_score || 0)) === "neutral",
    ).length,
    negative: rows.filter(
      (row) =>
        getSentimentBand(Number(row.sentiment_score || 0)) === "negative",
    ).length,
  };
  ...
  const topEntities = (Array.from(entityCounts.entries()) as [string, number][])
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
    .map((entry) => ({ name: entry[0], count: entry[1] }));

  const strongestFocusAreas = Array.from<[string, number]>(
    rows.reduce((acc, row) => {
      const key = sanitizeText(
        row.subheading || match.subheading || "General",
        80,
      );
...
function summarizeFanSentiment(lines: string[]) {
  ...
  const normalized = lines.length
    ? clamp(score / Math.max(lines.length * 2, 1), -1, 1)
    : 0;
  return {
    score: normalized,
    label:
...
function inferFailureStates(stats: any, fanFeedback: string[]) {
  ...
  const heatmapsText =
    `${stats?.heatmaps?.team || ""} ${stats?.heatmaps?.opponent || ""}`.toLowerCase();

  if (
    lowerFeedback.includes("nervous playing from the back") ||
    lowerFeedback.includes("distribution") ||
    passAccuracy < 82
  ) {
    failures.push({
      key: "build_up_risk",
      ...
      reason:
        "Fan comments point to discomfort in goalkeeper or first-phase distribution, and the passing base was not secure enough.",
      solution:
        "Implement a safer first-phase build-up pattern with a rest-defense triangle and one-touch exit options around the goalkeeper.",
    });
  }

  if (
    lowerFeedback.includes("late") ||
    lowerFeedback.includes("95th") ||
    heatmapsText.includes("late") ||
    xgAgainst > 1.5
  ) {
    failures.push({
      ...
      reason:
        "The match profile suggests control dropped late, leaving space for counters or chaotic defending in the closing phase.",
      solution:
        "Implement staggered rest-defense and late-game game-state drills to protect central transitions after turnovers.",
    });
  }
  ...
  if (xgFor < xgAgainst) {
    failures.push({
      ...
      reason:
        "The team generated less expected threat than the opponent, which suggests attacking structure or shot quality issues.",
      solution:
        "Drill a midfield double pivot into final-third release patterns to improve stable access into high-value shooting zones.",
    });
  }
  ...
  if (failures.length === 0) {
    failures.push({
      ...
      reason:
        "The data and fan voice suggest the result relied more on moments than repeatable control.",
      solution:
        "Reinforce controlled possession and counter-press triggers so the team can repeat positive match states more consistently.",
    });
  }
...
function buildPerceptionGaps(stats: any, fanFeedback: string[]) {
  ...
  for (const player of playerKpis) {
    ...
    const relatedFeedback = fanFeedback.filter((line) =>
    ...
    if (
      (feedbackBlob.includes("nervous") || feedbackBlob.includes("panic")) &&
      passAccuracy >= 85
    ) {
      gaps.push({
        playerOrTheme: playerName,
...
function buildHybridFeedbackReport(
  structuredStats: any,
  fanFeedback: string[],
) {
  ...
  const matchQualityScore = Math.round(
    clamp(
      55 + xgDiff * 12 + (passAccuracy - 80) * 1.5 + fanSentiment.score * 15,
      0,
      100,
    ),
  );
...
function buildNarrativeHybridReport(match: any, rows: any[]) {
  const total = rows.length;
  const avgSentiment = total
    ? rows.reduce((sum, row) => sum + safeNumber(row.sentiment_score), 0) /
      total
    : 0;
  const avgMagnitude = total
    ...
    : 0;
  const avgCredibility = total
    ? rows.reduce((sum, row) => sum + safeNumber(row.credibility_score), 0) /
      total
    : 0;
  ...
  const sortedPositive = [...rows]
    ...
    .sort(
      (a, b) => safeNumber(b.sentiment_score) - safeNumber(a.sentiment_score),
    )
    .slice(0, 2);
  const sortedNegative = [...rows]
    .filter((row) => safeNumber(row.sentiment_score) <= -0.2)
    .sort(
      (a, b) => safeNumber(a.sentiment_score) - safeNumber(b.sentiment_score),
    )
    .slice(0, 2);
  ...
  const { primaryTeam, secondaryTeam } = splitMatchTeams(
    match.opponent || "",
    match.club_name || "",
  );
  const statusHeader =
  ...
  return {
    ...
    metrics: {
      ...
      matchQualityScore: Math.round(
        clamp(60 + avgSentiment * 20 + avgCredibility * 20, 0, 100),
      ),
...
async function postGoogleCloudJson<T>(
  url: string,
  ...
  if (!response.ok) {
    const detail = (await response.text()).slice(0, 400);
    throw new Error(
      `Google Cloud request failed (${response.status}): ${detail}`,
    );
  }
...
async function analyzeNaturalLanguageText(text: string, languageCode?: string) {
  ...
  return {
    language:
      sentimentResponse.language ||
      entitiesResponse.language ||
      languageCode ||
      "und",
    ...
    entities: (entitiesResponse.entities || [])
      ...
      .filter((entity) => {
        ...
        if (/^(one|two|three|four|five|six|seven|eight|nine|ten)$/i.test(name))
          return false;
...
function getLinkExpiry(hours?: number) {
  const safeHours = Math.max(
    1,
    Math.min(Number(hours || DEFAULT_LINK_EXPIRY_HOURS), 168),
  );
...
async function verifyToken(token: string) {
  ...
  if (JWKS) {
    try {
      ...
      console.warn(
        "Primary JWKS verification failed, trying dynamic fallback:",
        jwksErr.message,
      );
    }
  }
...
async function requireUserRecord(req: express.Request, res: express.Response) {
  ...
  if (users.length === 0) {
    res
      .status(403)
      .json({ error: "User record not found. Please sign in again." });
    return null;
  }
  return users[0];
}
    ...
    (
      ...
      if (error?.type === "entity.too.large") {
        return res.status(413).json({
          error:
            "Upload is too large. Please choose a smaller image or shorter request payload.",
        });
      }
...
async function analyzeFeedback(text: string, language: string) {
  ...
  try {
    ...
    const reason =
      error instanceof Error ? error.message : "Unknown Google Cloud error";
    console.error(
      "Google Cloud feedback analysis failed, using heuristic fallback:",
      reason,
    );
    const credibilityScore = computeCredibilityScore({
      text,
...
async function updateUserCredibility(
  userId: string,
  feedbackCredibility: number,
  wasFlagged: boolean,
) {
  const existing =
    await sql`SELECT credibility_score FROM users WHERE id = ${userId}`;
...
async function backfillFeedbackAnalysisForClub(clubId: string, force = false) {
  ...
  for (const row of staleFeedback as any[]) {
    ...
    if (
      clubContext.isCrossClubFeedback &&
      !qualityFlags.includes("CROSS_CLUB_FEEDBACK")
    ) {
    ...
    const validationStatus =
      analysis.credibilityScore < 0.35 ? "FLAGGED" : "APPROVED";
...
app.post(
  "/api/auth/sync",
  rateLimit({ namespace: "auth-sync", limit: 30, windowMs: 60 * 60 * 1000 }),
  authenticate,
  async (req, res) => {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ error: "Authenticated user is missing an ID." });
    }
        ...
        const nextStatus: UserStatus =
          existing.status === "BLOCKED"
            ...
            : clubAdmin.length > 0 && clubAdmin[0].status !== "APPROVED"
              ...
              : "ACTIVE";
        const nextClubId =
          clubAdmin.length > 0 ? clubAdmin[0].id : existing.club_id;
        await sql`
        UPDATE users
        SET id = ${userId},
            email = ${email || null},
...
// Register Club
app.post(
  "/api/admin/register-club",
  rateLimit({ namespace: "register-club", limit: 5, windowMs: 60 * 60 * 1000 }),
  authenticate,
  async (req, res) => {
    ...
    if (!email) {
      return res.status(400).json({
        error:
          "Your account must have a valid email address before registering a club.",
      });
    }

    if (req.body.logoUrl && !logoUrl) {
      return res.status(400).json({
        error: "Logo upload is invalid. Please choose a smaller image file.",
      });
    }
    ...
    try {
      const existingUserById =
        await sql`SELECT * FROM users WHERE id = ${user.sub}`;
      const existingUserByEmail =
        await sql`SELECT * FROM users WHERE email = ${email}`;
      const existingUser =
        existingUserById[0] || existingUserByEmail[0] || null;

      if (existingUser?.club_id) {
        return res.status(400).json({
          error:
            "You already have a club registration in progress or assigned.",
        });
      }
      ...
      if (existingClub.length > 0) {
        return res.status(400).json({
          error:
            "A club with this name, license, or admin email already exists.",
        });
      }
    ...
    `;
      ...
      res.json({
        ...
        message:
          "Your club is under review. Approval typically takes 24 to 72 hours.",
      });
    } catch (error) {
      console.error("Club registration error:", error);
      res.status(500).json({
        error:
          error instanceof Error ? error.message : "Failed to register club",
      });
    }
  },
);
...
app.get("/api/clubs/my-request", authenticate, async (req, res) => {
  ...
  if (!userRecord.club_id) {
    return res
      .status(404)
      .json({ error: "No club registration found for this user." });
  }
...
app.post(
  "/api/super-admin/club-requests/:clubId/decision",
  authenticate,
  async (req, res) => {
    const superAdmin = await requireRole(req, res, ["SUPER_ADMIN"]);
    if (!superAdmin) return;
...
app.post("/api/admin/block-user", authenticate, async (req, res) => {
  ...
  if (!userId || typeof isBlocked !== "boolean") {
    return res
      .status(400)
      .json({ error: "userId and isBlocked are required." });
  }
...
app.post("/api/feedback", async (req, res) => {
  ...
  const subheading = normalizeSubheadingForTopic(
    topicType,
    req.body.subheading,
  );
  ...
  if (token) {
    try {
      ...
      const users =
        await sql`SELECT id, status, is_blocked, credibility_score FROM users WHERE id = ${verifiedUserId}`;
      if (users.length > 0) {
        const currentUser = users[0];
        if (currentUser.is_blocked || currentUser.status === "BLOCKED") {
          return res.status(403).json({
            error: "Your account has been blocked from submitting feedback.",
          });
        }
  ...
  if (!isAnonymous && !verifiedUserId) {
    return res.status(401).json({
      error: "Authenticated feedback requires a valid account session.",
    });
  }
  ...
  try {
    ...
    if (!match.club_is_active || match.club_status !== "APPROVED") {
      return res.status(403).json({
        error: "This club is not active for feedback collection yet.",
      });
    }
    ...
    if (match.audience === "AUTHENTICATED" && !verifiedUserId) {
      return res.status(403).json({
        error: "This feedback link only accepts authenticated users.",
      });
    }
    ...
    if (match.audience === "ANONYMOUS" && verifiedUserId && !isAnonymous) {
      return res
        .status(403)
        .json({ error: "This feedback link only accepts anonymous feedback." });
    }
...
app.get(
  "/api/admin/links/:sharableId/feedback",
  authenticate,
  async (req, res) => {
    const adminUser = await requireRole(req, res, ["ADMIN", "SUPER_ADMIN"]);
    if (!adminUser) return;
    ...
    `;
      ...
      if (
        ...
        adminUser.club_id !== match.club_id
      ) {
        return res.status(403).json({ error: "Forbidden" });
      }
...
app.post(
  "/api/admin/feedback/backfill-analysis",
  authenticate,
  async (req, res) => {
    const adminUser = await requireRole(req, res, ["ADMIN", "SUPER_ADMIN"]);
    if (!adminUser) return;
...
app.post(
  "/api/admin/links/:sharableId/summary",
  authenticate,
  async (req, res) => {
    const adminUser = await requireRole(req, res, ["ADMIN", "SUPER_ADMIN"]);
    if (!adminUser) return;
    ...
    `;
      ...
      let emailResult: { sent: boolean; reason?: string; id?: string } = {
        sent: false,
      };
      ...
      if (sendEmail) {
        if (!match.admin_email) {
          return res.status(400).json({
            error: "This club does not have an admin email configured.",
          });
        }
      ...
      res.status(500).json({
        error:
          error instanceof Error
            ? error.message
            : "Failed to generate link summary.",
      });
    }
  },
);
...
app.post("/api/admin/hybrid-report", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (feedbackRows.length === 0) {
      return res.status(400).json({
        error: "No feedback has been collected for this sharable link yet.",
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to generate hybrid feedback report." });
  }
});
...
app.post("/api/admin/hybrid-report/email", authenticate, async (req, res) => {
  ...
  if (!report || typeof report !== "object") {
    return res
      .status(400)
      .json({ error: "Hybrid report payload is required." });
  }

  try {
    const clubs =
      await sql`SELECT name, admin_email FROM clubs WHERE id = ${clubId} LIMIT 1`;
    if (clubs.length === 0) {
    ...
    const club = clubs[0];
    const recipient =
      emailTo || sanitizeText(club.admin_email, 255).toLowerCase();
    if (!recipient) {
      return res
        .status(400)
        .json({ error: "No club admin email is configured for this report." });
    }
    ...
    const text = [
      ...
      ...(report.painPoints || []).map(
        (item: any) => `- ${item.title}: ${item.reason}`,
      ),
      "",
      "Tactical Fixes:",
      ...(report.actionItems?.tacticalFixes || []).map(
        (item: string) => `- ${item}`,
      ),
      "",
      "Communication Fixes:",
      ...(report.actionItems?.communicationFixes || []).map(
        (item: string) => `- ${item}`,
      ),
      "",
      "Questions for Strategy:",
      ...(report.questionsForStrategy || []).map(
    ...
    res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Failed to send hybrid report email.",
    });
  }
});
...
app.get("/api/matches/:sharableId", async (req, res) => {
  ...
  try {
    ...
    if (
      match[0].expires_at &&
      new Date(match[0].expires_at).getTime() < Date.now()
    ) {
...
app.post(
  ...
  rateLimit({
    ...
    windowMs: 60 * 60 * 1000,
    keyResolver: (req) =>
      (req as any).user?.sub ? String((req as any).user.sub) : null,
  }),
  authenticate,
  async (req, res) => {
    const adminUser = await requireRole(req, res, ["ADMIN", "SUPER_ADMIN"]);
    ...
    if (!clubId || !opponent) {
      return res
        .status(400)
        .json({ error: "clubId and opponent are required." });
    }
    ...
    const clubs =
      await sql`SELECT status, is_active FROM clubs WHERE id = ${clubId}`;
    if (clubs.length === 0) {
      return res.status(404).json({ error: "Club not found." });
    }
    if (!clubs[0].is_active || clubs[0].status !== "APPROVED") {
      return res.status(403).json({
        error: "Club must be approved before creating sharable links.",
      });
    }
