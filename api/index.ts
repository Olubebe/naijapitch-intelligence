...
function buildLinkDigest(match: any, rows: any[]) {
  ...
  const topEntities = Array.from<[string, number]>(entityCounts.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
...
type FeedbackThemeInsight = FeedbackThemeDefinition & {
  count: number;
  avgSentiment: number;
  avgMagnitude: number;
  examples: string[];
...
const FEEDBACK_THEME_LIBRARY: FeedbackThemeDefinition[] = [
  {
    key: 'build_up',
    label: 'Build-up and Distribution',
    summary: 'supporters repeatedly focused on how secure the team looked when building from deep areas',
    solution: 'Install a calmer first-phase build-up with a clearer pivot angle, a goalkeeper-safe outlet, and one designated long-release pattern when the press jumps.',
    communication: 'Explain whether the risky build-up pattern was intentional and what the team expects from the goalkeeper and first receiver under pressure.',
    question: 'Is the current build-up structure creating genuine control, or is it asking one player to absorb too much risk alone?',
    opponentLens: 'the opponent looked most dangerous whenever the first line was pressed aggressively',
    keywords: ['goalkeeper', 'keeper', 'distribution', 'playing from the back', 'build-up', 'build up', 'sanchez', 'press'],
  },
  {
    key: 'transition_control',
    label: 'Transition Control',
    summary: 'the feedback kept returning to transition moments, counters, and how exposed the team looked after turnovers',
    solution: 'Tighten rest-defense with a staggered fullback line and one holding midfielder locked to the counter lane whenever attacks break down.',
    communication: 'Clarify whether the late-game shape was supposed to chase the win or protect against transition damage.',
    question: 'Which rest-defense rule broke first when the game became stretched, and how do we rehearse it under fatigue?',
    opponentLens: 'the opponent was allowed to believe transition attacks would open the game',
    keywords: ['counter', 'transition', 'break', 'turnover', 'late goal', '95th', 'exposed', 'high line', 'space behind'],
  },
  {
    key: 'finishing',
    label: 'Chance Conversion',
    summary: 'supporters highlighted the gap between getting into good zones and actually finishing the decisive action',
    solution: 'Run high-pressure finishing scenarios with one-touch box entries and rebound reactions so good attacking phases do not end without punishment.',
    communication: 'Separate chance creation from chance conversion so supporters understand whether the process worked even when the final shot did not.',
    question: 'Are good attacking sequences ending with the right shooter in the right zone, or are promising attacks dying on the wrong decision?',
    opponentLens: 'the opponent survived good pressure because the final action lacked precision',
    keywords: ['finish', 'finishing', 'clinical', 'chance', 'chances', 'miss', 'missed', 'sitter', 'conversion', 'final ball', 'shot'],
  },
  {
    key: 'midfield_control',
    label: 'Midfield Control',
    summary: 'a large share of comments were really about who controlled the center of the pitch and second balls',
    solution: 'Use a double-pivot or tighter midfield staggering so the primary controller is not isolated when the match pace spikes.',
    communication: 'Frame midfield roles more clearly so one player is not blamed for structural spacing issues created around him.',
    question: 'Did the midfield shape protect the ball-winning player, or did it leave him carrying the whole game-state alone?',
    opponentLens: 'the opponent gained hope whenever central control loosened and second balls became contested',
    keywords: ['midfield', 'pivot', 'caicedo', 'rodri', 'szoboszlai', 'center', 'middle', 'second ball', 'bypass', 'overrun'],
  },
  {
    key: 'game_management',
    label: 'Game Management',
    summary: 'the overall tone suggests supporters judged the performance through the team’s ability to calm the match after momentum swings',
    solution: 'Build a late-game control menu: one possession pattern, one territory pattern, and one pressure-release pattern that the team can trigger automatically.',
    communication: 'Explain the intended game-state plan after the 60th minute so supporters understand whether the team was protecting, pressing, or improvising.',
    question: 'When momentum turned, did the staff have a visible control mechanism, or was the match left to emotion and improvisation?',
    opponentLens: 'the opponent was encouraged by the sense that the match could still tilt late on',
    keywords: ['late', 'control', 'game management', 'manage', 'dropped too deep', 'chaos', 'panic', 'suffer', 'rollercoaster', 'calm'],
  },
  {
    key: 'coaching_changes',
    label: 'Coaching and Adjustments',
    summary: 'some of the strongest opinions were about substitutions, tactical tweaks, and whether the bench changed the direction of the match in time',
    solution: 'Pre-plan substitution chains by game state so personnel changes protect the structure instead of chasing the feeling of the moment.',
    communication: 'Address the logic behind substitutions directly, especially when supporters feel the game drifted before the bench reacted.',
    question: 'Did the substitutions solve the real problem on the pitch, or only respond to the loudest symptom?',
    opponentLens: 'the opponent benefited if the tactical tweaks arrived a phase too late',
    keywords: ['sub', 'subs', 'substitution', 'manager', 'coach', 'maresca', 'adjustment', 'bench', 'change', 'changes'],
  },
];

function getFeedbackRowText(row: any) {
  return sanitizeText(row.translated_text || row.original_text || '', 1000);
}

function getFeedbackQuote(row: any, maxLength = 180) {
  const text = getFeedbackRowText(row);
  if (!text) return '';
  return sanitizeText(text.replace(/\s+/g, ' '), maxLength);
}

function buildThemeInsights(rows: any[]) {
  return FEEDBACK_THEME_LIBRARY.map((theme) => {
    const matchingRows = rows.filter((row) => {
      const lower = getFeedbackRowText(row).toLowerCase();
    ...
    const sortedExamples = [...matchingRows]
      .sort((a, b) => safeNumber(b.magnitude) - safeNumber(a.magnitude))
      .slice(0, 2)
      .map((row) => getFeedbackQuote(row))
      .filter(Boolean);

    const avgSentiment = matchingRows.reduce((sum, row) => sum + safeNumber(row.sentiment_score), 0) / matchingRows.length;
    const avgMagnitude = matchingRows.reduce((sum, row) => sum + safeNumber(row.magnitude), 0) / matchingRows.length;
    ...
    } satisfies FeedbackThemeInsight;
  })
    .filter((theme): theme is FeedbackThemeInsight => Boolean(theme))
    .sort((a, b) => {
      if (b.count !== a.count) return b.count - a.count;
      return b.avgMagnitude - a.avgMagnitude;
    });
}
...
function buildNarrativeHybridReport(match: any, rows: any[]) {
  ...
  const loudestRows = [...rows]
    .sort((a, b) => safeNumber(b.magnitude) - safeNumber(a.magnitude))
    .slice(0, 3);
  const themeInsights = buildThemeInsights(rows);
  const primaryThemes = themeInsights.slice(0, 3);
  const negativeThemes = [...themeInsights]
    .filter((theme) => theme.avgSentiment < 0.1 || theme.avgMagnitude >= avgMagnitude || theme.count >= 2)
    .slice(0, 2);
  ...
  const leadingTheme = primaryThemes[0];
  const secondaryTheme = primaryThemes[1];
  const positiveTheme = [...themeInsights]
    .filter((theme) => theme.avgSentiment >= 0.15)
    .sort((a, b) => b.avgSentiment - a.avgSentiment || b.count - a.count)[0];
  const opponentTheme = [...themeInsights]
    .sort((a, b) => b.avgMagnitude - a.avgMagnitude || b.count - a.count)[0];

  const why = [
    `The feedback indicates that ${primaryTeam}'s display was shaped by ${leadingTheme ? leadingTheme.summary : 'the repeated emotional themes inside the supporter comments'}, rather than by one isolated moment. This summary is collated from ${total} feedback entr${total === 1 ? 'y' : 'ies'} tied to this shareable link, using the stored Google Cloud sentiment and magnitude scores from each response.`,
    `The Positive: ${positiveTheme ? `${capitalizeFirstLetter(positiveTheme.label.toLowerCase())} came through as a clear positive thread, with ${positiveTheme.count} comment${positiveTheme.count === 1 ? '' : 's'} pointing to how ${positiveTheme.summary}. The strongest positive fan voice was ${positiveQuote} (Ref: Fan Report), while the average sentiment across the link settled at ${avgSentiment.toFixed(2)} with an intensity of ${avgMagnitude.toFixed(2)} (Ref: Google Cloud Sentiment).` : `Fans still found encouraging moments, anchored by ${positiveQuote} (Ref: Fan Report), and the average sentiment landed at ${avgSentiment.toFixed(2)} with intensity at ${avgMagnitude.toFixed(2)} (Ref: Google Cloud Sentiment).`}`,
    `The Negative: ${negativeThemes.length > 0 ? negativeThemes.map((theme) => `${capitalizeFirstLetter(theme.label.toLowerCase())} emerged as a pain point in ${theme.count} piece${theme.count === 1 ? '' : 's'} of feedback, suggesting that ${theme.summary}`).join(' ') : 'The sharpest criticism was less about one player and more about how unstable the match felt when pressure rose.'} The clearest negative fan voice was ${negativeQuote} (Ref: Fan Report).`,
    `The ${secondaryTeam} Perspective: From the collected supporter comments, ${secondaryTeam} were perceived as a threat whenever ${opponentTheme ? opponentTheme.opponentLens : 'the game stopped looking settled and controlled'}. The loudest emotional reaction from the link was ${emotionalLine} (Ref: Google Cloud Magnitude).`,
    `Collated Summary: The overall credibility of this feedback set is ${(avgCredibility * 100).toFixed(0)}%, and the repeated themes were ${primaryThemes.length > 0 ? primaryThemes.map((theme) => theme.label.toLowerCase()).join(', ') : 'general match control and supporter confidence'}. This means the report is responding to the actual concerns supporters repeated most often, not a hardcoded match script.`,
  ].join('\n\n');

  const tacticalFixes = [
    `For ${primaryTeam}: "${leadingTheme ? leadingTheme.label : 'Kill the Chaos'}"`,
    ...(negativeThemes.length > 0
      ? negativeThemes.map((theme) => `${capitalizeFirstLetter(theme.label.toLowerCase())}: ${theme.solution}`)
      : [
        ...
        `Late-game pressure management needs a relief pattern so the team can escape pressure without forcing risky short passes.`,
      ]),
    `For ${secondaryTeam}: "${secondaryTheme ? secondaryTheme.label : 'Turn Pressure into Punishment'}"`,
    `${secondaryTeam} can learn from the same supporter evidence by targeting the moments when ${opponentTheme ? opponentTheme.opponentLens : 'the match opens up emotionally and structurally'}.`,
  ];

  const communicationFixes = [
    `Address the most emotional phase of the match directly, especially because the average magnitude for this link was ${avgMagnitude.toFixed(2)} and the loudest supporter reaction was ${emotionalLine}.`,
    ...(negativeThemes.length > 0
      ? negativeThemes.map((theme) => theme.communication)
      : [`Explain whether the team was intentionally protecting the lead or whether control was genuinely lost, so supporters understand the game-state choices.`]),
    `Use player-specific communication carefully when one name dominates the conversation; separate role design from individual blame.`,
  ];
  ...
  const questionsForStrategy = [
    ...
    ...(primaryThemes.slice(0, 2).map((theme) => theme.question)),
  ...
  const painPoints = (
    negativeThemes.length > 0
      ? negativeThemes.map((theme) => ({
        title: theme.label,
        reason: `${capitalizeFirstLetter(theme.summary)}. This theme appeared in ${theme.count} feedback entr${theme.count === 1 ? 'y' : 'ies'} with an average sentiment of ${theme.avgSentiment.toFixed(2)} and intensity of ${theme.avgMagnitude.toFixed(2)}.${theme.examples[0] ? ` Example: "${theme.examples[0]}"` : ''}`,
      }))
      : [
        {
