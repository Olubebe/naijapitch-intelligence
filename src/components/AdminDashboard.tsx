...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ data, onBlockUser }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap gap-2 p-1 bg-gray-100 rounded-xl w-full sm:w-fit">
        {['All', 'Match', 'Players', 'Transfers'].map((tab) => (
          <button
            ...
            className={`flex-1 sm:flex-none px-4 sm:px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === tab ? 'bg-white text-green-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
            {tab}
          </button>
        ))}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="lg:hidden p-4 space-y-4">
          {filteredData.map((f) => {
            ...
            const Icon = s.icon;
            return (
              <div key={f.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="text-sm font-bold text-gray-900">{f.opponent}</div>
                    <div className="flex flex-wrap gap-2">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${s.color}`}>
                        <Icon className="w-3 h-3" />
                        {s.label}
                      </span>
                      <span className="text-xs font-bold text-gray-500 uppercase tracking-wider bg-white px-2 py-0.5 rounded border">
                        {f.category}
                      </span>
                      <span className="text-xs font-semibold text-gray-600 bg-amber-50 px-2.5 py-1 rounded-full border border-amber-100">
                        {f.subheading || 'General'}
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-gray-600">{(f.credibility_score * 100).toFixed(0)}%</span>
                    </div>
                  </div>
                  <div>
                    ...
                    {!f.is_anonymous && f.is_blocked && (
                      <div className="text-[9px] font-black text-red-500 uppercase tracking-widest">Blocked</div>
                    )}
                  </div>
                </div>

                {f.is_cross_club_feedback && (
                  <div className="rounded-xl border border-red-100 bg-red-50 px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-red-700">
                    Cross-club: {f.primary_subject_club_name || 'Different club'}
                  </div>
                )}

                <div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-gray-400">Feedback</div>
                  <p className="mt-1 text-sm leading-relaxed text-gray-700 line-clamp-4">
                    {f.translated_text}
                  </p>
                </div>
              </div>
          ...
          {filteredData.length === 0 && (
            <div className="rounded-2xl border border-gray-100 bg-gray-50 p-6 text-center text-gray-400">
              No feedback found in this category.
            </div>
          )}
        </div>
      ...
      {selectedFeedback && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-md animate-in fade-in zoom-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 sm:p-6 bg-gray-50 border-b flex justify-between items-center gap-3">
            ...
            <div className="p-4 sm:p-8 space-y-6 overflow-y-auto">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-2xl border bg-gray-50">
                {(() => {
            ...
            <div className="p-4 sm:p-6 border-t flex justify-end">
              <button onClick={() => setSelectedFeedback(null)} className="w-full sm:w-auto bg-gray-900 text-white font-bold px-8 py-2.5 rounded-xl hover:bg-black transition-all">Close Report</button>
            </div>
          </div>
        </div>
      )}
